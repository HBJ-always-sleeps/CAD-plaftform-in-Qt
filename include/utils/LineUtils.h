...
public:
     ...
     * 延长线两端
     * 
     * 对应Python: engine_cad_v3.py 第101-114行
     * 
     * @param line 输入线段
     * @param dist 延长距离
     * @return 延长后的线段
     */
    static Line2D extend(const Line2D &line, double dist) {
        const QVector<Point2D> &coords = line.points;
        ...
        QVector<Point2D> newCoords = coords;
        
        // 起端延长
        Point2D p1(coords[0]);
        Point2D p2(coords[1]);
        Point2D vec1(p1.x - p2.x, p1.y - p2.y);
        double mag1 = std::sqrt(vec1.x * vec1.x + vec1.y * vec1.y);
        if (mag1 > 0) {
            vec1.x /= mag1;
            vec1.y /= mag1;
        }
        Point2D newStart(p1.x + vec1.x * dist, p1.y + vec1.y * dist);
        newCoords.insert(0, newStart);
        
        // 终端延长
        Point2D pn1(coords[coords.size() - 2]);
        Point2D pn(coords[coords.size() - 1]);
        Point2D vec2(pn.x - pn1.x, pn.y - pn1.y);
        double mag2 = std::sqrt(vec2.x * vec2.x + vec2.y * vec2.y);
        if (mag2 > 0) {
            vec2.x /= mag2;
            vec2.y /= mag2;
        }
        Point2D newEnd(pn.x + vec2.x * dist, pn.y + vec2.y * dist);
        newCoords.append(newEnd);
        
        return Line2D(newCoords);
    }
     ...
     * 完整实现需要复杂的拓扑分析
     */
    static Line2D mergeLines(const QVector<Line2D> &lines) {
