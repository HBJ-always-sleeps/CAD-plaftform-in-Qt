...
{
    setWindowTitle("航道断面算量自动化平台 Qt C++版本 v1.0");
...
QWidget* MainWindow::createHeader()
{
    ...
    // 版本信息
    QLabel *versionLabel = new QLabel("v1.0 | C++整合版", header);
...
void MainWindow::createAutosectionBackfillParams(QGridLayout *layout)
{
    ...
    layout->addWidget(m_paramCombinedCalcMode, 2, 1);
    layout->addWidget(m_paramCombinedDistinguishDesign, 3, 0, 1, 2);
...
{
    QStringList tasks = {"autoline", "autopaste", "autohatch", "autosection", "backfill", "autosection_backfill"};
